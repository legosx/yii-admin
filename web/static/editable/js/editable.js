...
(function ($) {
    ...
    Editable.prototype = {
        ...
        toggle: function (show, delay) {
            var self = this, $value = self.$value, $inline = self.$inline, $input = self.$input;
            if (show) {
                if (!self.asPopover) {
                    $value.hide();
                    $inline.show();
                    ...
                    if ($input.get(0).tagName.toLowerCase() == 'textarea') {
                        if (!$input.data('textarea-autosize-enabled')) {
                            $input.data('textarea-autosize-enabled', true);
                            autosize($input);
                        }
                    ...
                    var $save_buttons = $input.parents('tr').find('.save-buttons');
